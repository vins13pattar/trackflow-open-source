...
| Two-way device commands (immobilize, etc.) | **DONE** | Generic commands queue with state machine (queued→sent→acked/failed/expired/canceled). Teltonika Codec 12 wire encoder available for delivery via the open device socket. |
...
- [x] **Two-way commands** to devices (immobilize, mobilize, request_location, set_interval, reboot) ✅ fully complete: `device_commands` table with state machine (`queued → sent → acked|failed|expired|canceled`); tenant-facing `POST/GET/DELETE /devices/:id/commands`; ingest-only pickup + ack endpoints; GT06 0x80 wire encoder + 0x15 response parser; Teltonika Codec 12 encoder + response parser. All gated tests pass.
...
- [x] **CI security gates** — `.github/workflows/security.yml`: pnpm audit (fail on high), Semgrep SAST (`p/ci` + `p/typescript`), gitleaks full-history secret scan, CycloneDX SBOM artifact; weekly schedule + PRs; Dependabot (npm grouped minor/patch + actions). (First green run on GitHub pending.)
- [x] **Next.js 16 upgrade** (15 was already EOL-bound; jumped straight to 16.2 + React 19.2) — clears the Next DoS/SSRF advisories from §5; vitest bumped to v3 + a `shell-quote` override so **`pnpm audit --audit-level high` is clean** (5 transitive moderates remain, all dev-tooling paths); `next lint` script removed (deleted in Next 16; no eslint config existed). Typecheck + build green; **full browser regression pass still pending** (also covers the new CSP).
...
- [~] **Multi-instance ingest**: NLB/Fly TCP load balancing → N listeners + **Redis-backed session map keyed on IMEI** (reconnects land anywhere); two-way command delivery routed to the pod holding the socket. **Building block shipped**: `apps/ingest/src/presence.ts` — an `imei → { instanceId, lastSeen }` presence registry with the proven dual store (in-memory default / Upstash-REST gated on `UPSTASH_REDIS_REST_*`), TTL'd keys so a crashed pod's bindings self-expire, and take-over-on-reconnect semantics (a stale close from the old instance can't evict the newer binding). Wired into the TCP server (bind on IMEI discovery, refresh on every fix, release own binding on close) under a stable `INGEST_INSTANCE_ID` (FLY_MACHINE_ID/HOSTNAME fallback). 8 unit tests (memory + stubbed-fetch Upstash). The store now lives in `@trackflow/shared` so **the API reads the same registry**: `apps/api/src/presence.ts` (`lookupPresence`, advisory + never-throws) annotates the command-queue response with `delivery: { connected, instanceId }` so a caller knows whether a command delivers promptly or waits for reconnect; degrades to `connected:false` in single-instance dev (no shared store). 3 API + 1 ingest-factory tests. **Remaining**: the LB/topology config, and the holder actually being *pushed* the command (needs each instance to expose a reachable "drain now" endpoint registered in presence) — today commands still drain via the existing poll-on-connect path, which already works across instances since presence isn't required for correctness, only for push latency.
...
- [ ] **SSE fan-out across instances**: Redis pub/sub bridge so any API instance serves any tenant's stream; document the Workers-portable path.
