...
describe.skipIf(!enabled)('device groups: CRUD + group-targeted geofence dispatch', () => {
  ...
  it('a geofence targeting a group dispatches to that group\'s members', async () => {
    ...
    const [g] = await withSystem(db, 'test-fixture', (tx) => tx.insert(deviceGroups).values({ tenantId, name: 'East', color: '#abcdef' }).returning());
    await withSystem(db, 'test-fixture', (tx) => tx.insert(deviceGroupMembers).values({ tenantId, groupId: g!.id, deviceId: deviceA }));
    // Geofence targeting only that group (allDevices=false, no explicit deviceIds).
    const [zone] = await withSystem(db, 'test-fixture', (tx) => tx.insert(geofences).values({
    ...
    try {
      ...
      const firedA = await withSystem(db, 'test-fixture', (tx) => evaluateGeofences(tx, {
      ...
      const firedB = await withSystem(db, 'test-fixture', (tx) => evaluateGeofences(tx, {
        tenantId, deviceId: deviceB, lat: 12.97, lon: 77.59, fixTime: new Date(),
      }));
      expect(firedB).toHaveLength(0);
    } finally {
      await withSystem(db, 'test-fixture', (tx) => tx.delete(geofenceStates).where(eq(geofenceStates.geofenceId, zone!.id)));
      await withSystem(db, 'test-fixture', (tx) => tx.delete(geofences).where(eq(geofences.id, zone!.id)));
      await withSystem(db, 'test-fixture', (tx) => tx.delete(deviceGroups).where(eq(deviceGroups.id, g!.id)));
    }
  });
});
