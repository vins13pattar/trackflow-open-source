...
describe('base32', () => {
  ...
  it('encodes the RFC secret', () => {
    expect(base32Encode(new TextEncoder().encode('12345678901234567890'))).toBe(RFC_SECRET_B32);
  });
});
...
describe('totp (RFC 6238 SHA-1 vectors)', () => {
  ...
  it('matches the published 8-digit vectors', async () => {
    for (const [t, expected] of vectors) {
      expect(await totpCode(RFC_SECRET_B32, { digits: 8, timestampMs: t * 1000 })).toBe(expected);
    }
  });

  it('6-digit codes are the truncation of the 8-digit ones', async () => {
    expect(await totpCode(RFC_SECRET_B32, { timestampMs: 59 * 1000 })).toBe('287082');
  });

  it('verifies within the skew window and rejects outside it', async () => {
    const now = 1111111111 * 1000;
    const code = await totpCode(RFC_SECRET_B32, { timestampMs: now });
    expect(await verifyTotp(RFC_SECRET_B32, code, { timestampMs: now })).toBe(true);
    // One step earlier still passes (±1 window)…
    expect(await verifyTotp(RFC_SECRET_B32, code, { timestampMs: now + 30_000 })).toBe(true);
    // …two steps later does not.
    expect(await verifyTotp(RFC_SECRET_B32, code, { timestampMs: now + 90_000 })).toBe(false);
    expect(await verifyTotp(RFC_SECRET_B32, '000000', { timestampMs: now })).toBe(false);
    expect(await verifyTotp(RFC_SECRET_B32, 'not-a-code', { timestampMs: now })).toBe(false);
  });
});
...
describe('secrets, URIs and recovery codes', () => {
  ...
  it('builds an otpauth URI apps can parse', () => {
    const uri = otpauthUri({ secret: RFC_SECRET_B32, accountName: 'a@b.co', issuer: 'TrackFlow' });
